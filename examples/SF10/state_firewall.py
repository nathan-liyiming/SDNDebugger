...
class StatefulFirewall(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        if buffer_id:
            ...
            datapath=datapath, match=match, buffer_id=buffer_id, cookie=0,
    ...
    def _packet_in_handler(self, ev):
        ...
        if (in_port == 1) and (pkt_icmp != None):
            ...
            match = parser.OFPMatch(in_port=1,dl_type=0x0800,nw_proto=1)
            ...
            out_port = 1
            actions = [parser.OFPActionOutput(out_port)]
            match = parser.OFPMatch(in_port=2,dl_type=0x0800,nw_proto=1)
            if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                self.add_flow(datapath, 1, match, actions, msg.buffer_id)
            else:
                self.add_flow(datapath, 1, match, actions)

            # *** Line was originally wrong in implementation; commented out to demo debugger. ***
