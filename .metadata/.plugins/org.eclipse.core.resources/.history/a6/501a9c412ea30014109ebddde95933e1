...
public class StatefulFirewallMonitorHandler implements Runnable {
											...
											public Observable<Notification<Void>> call(
												...
												for (String s : temp) {
													...
													if (s.contains("ICMP")|| s.contains("OF")) {
														if (!(s.contains("of_echo_request") || s.contains("of_echo_reply")))
	...
	private boolean Oracle(String pkt) {
		if (flag) {
			String assertion = this.checkPoints.peek();
			System.out.println("Expect to get:" + assertion);
	...
	private void initOracle() {
		...
		this.checkPoints.add("of_packet_out"); // arp response generated by the
												// controller
		this.checkPoints.add("Echo (ping) request"); // icmp ping request sent
														// from h1 arrive at the
														// switch
		this.checkPoints.add("of_packet_in"); // ping request was sent to the
												// controller
		this.checkPoints.add("of_flow_add"); // flow_mod for the switches for
												// bidirections
		this.checkPoints.add("of_flow_add"); // flow_mod for the switches for
												// bidirections
		this.checkPoints.add("of_packet_out"); // ping request sent to the
												// controller before sent to h2
		this.checkPoints.add("Echo (ping) request"); // ping request sent from
														// h1 reached h2
		this.checkPoints.add("of_packet_in"); // arp request sent from h2
		this.checkPoints.add("of_packet_out"); // arp response generated by the
												// controller
		this.checkPoints.add("Echo (ping) reply"); // ping reply arrive at the
													// switch
		this.checkPoints.add("Echo (ping) reply"); // ping reply arrive at the
													// h1
		// afterward flow
