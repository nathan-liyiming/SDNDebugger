...
public class ArpServer implements IFloodlightModule, IOFMessageListener {
	...
	// Serve for RouteFlow only: hard code ip and mac pair
	private HashSet<Integer> switchIP = new HashSet<Integer>();
	...
	// Serve for RouteFlow only
	private void addSwitchARP() {
		switchIP.add(IPv4.toIPv4Address("172.31.1.1"));
		switchIP.add(IPv4.toIPv4Address("172.31.2.1"));
		switchIP.add(IPv4.toIPv4Address("172.31.3.1"));
		switchIP.add(IPv4.toIPv4Address("172.31.4.1"));
	}
	...
	public boolean isCallbackOrderingPostreq(OFType type, String name) {
		return (OFType.PACKET_IN == type && name
				.equals(DeviceManagerImpl.MODULE_NAME));
	}
	...
	public net.floodlightcontroller.core.IListener.Command receive(
		...
		if (!deviceIterator.hasNext() && !switchIP.contains(targetIP)) {
			return Command.CONTINUE;
		}
		...
		byte[] deviceMac = null;
		if (switchIP.contains(targetIP)) {
			deviceMac = MACAddress.valueOf(65535).toBytes();
		} else {
			IDevice device = deviceIterator.next();
			deviceMac = MACAddress.valueOf(device.getMACAddress()).toBytes();
		}
		...
		// Send the ARP reply
		log.info(String.format("Sending ARP reply %s->%s", IPv4
				.fromIPv4Address(targetIP), MACAddress.valueOf(deviceMac)
				.toString()));
